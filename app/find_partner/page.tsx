...
export default function FindPartnerPage() {
  ...
  const onDislike = async () => {
    ...
    const { error } = await ((getSupabase() as any)
      .from('likes')
      .insert([{ from_user_id: currentUserId, to_user_id: currentUser.id, status: 'rejected' }]) as any)
  ...
  const createChatWithGoal = async (goal: Goal) => {
    ...
    const { error: likeError } = await ((getSupabase() as any)
      .from('likes')
      .insert([{ from_user_id: currentUserId, to_user_id: partnerToPropose.id, status: 'pending' }]) as any)
    ...
    const { data: existingChats, error: existingChatError } = await ((getSupabase() as any)
      .from('chats')
      .select('id,user1_id,user2_id')
      .or(existingChatFilter) as any)
    ...
    if (!chatId) {
      const { data: newChat, error: chatError } = await ((getSupabase() as any)
        .from('chats')
    ...
    const { error: messageError } = await ((getSupabase() as any)
      .from('messages')
      .insert([{ chat_id: chatId, sender_id: currentUserId, content: messageText }]) as any)
